    ...
    methods:{
     ...
     handleSizeChange(val) {
      ...
      frozenOrThaw:function(status){
        let _this = this;
        let url = window.g.videoMerchant;
    ...
    async  deleteRow(index, rows) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          type: 'warning'
               ...
               const api = window.g.videoMerchantdelete;
      ...
      //删除选中数据
      videoMerchantdelete(index, rows){
        const api = window.g.videoMerchantdelete;
        const date = new URLSearchParams();
              date.append("spIsDelete","1");
              date.append("id",rows.id)
     ...
     //   搜索
      ...
      // 获取数据
      ditch(){
        const api = window.g.videoMerchant;
        const date ={
            params:{
                current:this.number,
                size:this.present,
